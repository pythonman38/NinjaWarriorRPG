...
public class SwordSkillController : MonoBehaviour
{
    ...
    private bool isReturning, canRotate = true;
    ...
    private float hitTimer, hitCooldown, spinDirection;
    ...
    private void Update()
    {
        ...
        if (isReturning)
        {
            ...
            if (Vector2.Distance(transform.position, player.transform.position) < 0.5) player.CatchTheSword();
        }
    ...
    private void SpinLogic()
    {
        if (isSpinning)
        {
            if (Vector2.Distance(player.transform.position, transform.position) > maxTravelDistance && !wasStopped) StopWhenSpinning();
            ...
            if (wasStopped)
            {
                ...
                if (hitTimer < 0) hitTimer = hitCooldown;

                Collider2D[] colliders = Physics2D.OverlapCircleAll(transform.position, 1);
                foreach (var hit in colliders)
                {
                    if (hit.GetComponent<Enemy>()) SwordSkillDamage(hit.GetComponent<Enemy>());
                }
            }
        }
    }
    ...
    private void SetupTargetsForBounce(Collider2D collision)
    {
        if (collision.GetComponent<Enemy>())
        {
            if (isBouncing && enemyTarget.Count <= 0)
            {
                ...
                foreach (var hit in colliders)
                {
                    if (hit.GetComponent<Enemy>()) enemyTarget.Add(hit.transform);
                }
            }
        }
    }
